...
# Import YOLO v11 detector
try:
    from detector.yolov11_detector import YOLOv11Detector
    YOLO11_AVAILABLE = True
except ImportError:
    print("Warning: YOLO v11 detector not available. Install ultralytics package.")
...
class Detector:
    def __init__(self,
        ...
        # Initialize appropriate YOLO detector based on version
        if yolo_version == 'v11':
            if not YOLO11_AVAILABLE:
                raise ImportError("YOLO v11 is not available. Please install ultralytics package: pip install ultralytics")
            self.detector = YOLOv11Detector(custom=custom, conf_threshold=score, iou_threshold=iou)
            print(f"Initialized YOLO v11 Detector (custom={custom})")
        else:
            # Original YOLO v7 implementation
    ...
    def detect(self, frame):
        if self.yolo_version == 'v11':
            return self.detector.detect(frame)
        else:
            # Original YOLO v7 detection logic
